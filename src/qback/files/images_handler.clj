(ns qback.files.images-handler
  (:require [clojure.java.io :as io]
            [qback.files.image-processing :refer [image?]]
...
(defn- process-image [dest-file dest-file-path file file-name hash]
...
(defn image-upl-handler [req]
  (let [file-data (get-in req [:multipart-params "uplfile"])
        ...
        is-image (image? file)]
    (if is-image
      (process-image dest-file dest-file-path file file-name hash)
